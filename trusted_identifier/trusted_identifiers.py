...
def main():
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        print('%d/%d', (i, n_rows))
        identifier = TrustedIdentifier(name=row['name'], searched_name=row['searched_name'], value_founded=row['value_founded'] , trusted=row['trusted'])
        try:
    ...
    # this query should return 13182 rows
    print('count of records in table %s: %d' % (DataIdentifiersSelectedGeorge.__tablename__, count_data_from_trusted_identifers))
